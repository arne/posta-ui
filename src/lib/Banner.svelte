<script lang="ts">
  ...
  $: divClass = cn(position, divClass, divClasses[bannerType], $$props.classDiv);
  $: div2Class = cn(innerClass, insideDivClasses[bannerType], $$props.classInner);
  let show = true;
  $: handleHide = () => {
    show = !show;
  };
