<script lang="ts">
  ...
  import Wrapper from '../helpers/Wrapper.svelte';
  ...
  $: textareaClass = wrapped ? wrappedClass : cn(wrapperClass, unWrappedClass);

  const headerClass = (header: boolean) =>
    cn(header ? 'border-b' : 'border-t', 'py-2 px-3 border-gray-200 dark:border-gray-600');

  let innerWrapperClass: string;
  $: innerWrapperClass = cn(
